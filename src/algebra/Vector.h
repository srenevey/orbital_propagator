...
#ifndef ORBITAL_PROPAGATOR_VECTOR_H
#define ORBITAL_PROPAGATOR_VECTOR_H
...
public:
    ...
    template <typename U>
    friend Vector operator*(const double& a, const Vector<U,N>& v) {
        std::array<double, N> data;
        for (std::size_t i = 0; i < N; ++i)
            data[i] = a * v[i];
        return Vector(v.m_frame, data);
    }
    ...
    [[nodiscard]] double norm_inf() const {
        double norm_inf = std::abs(m_data[0]);
        for (std::size_t i = 1; i < N; ++i)
            norm_inf = std::max( norm_inf, std::abs(m_data[i]) );
        return norm_inf;
    }
...
#endif //ORBITAL_PROPAGATOR_VECTOR_H
