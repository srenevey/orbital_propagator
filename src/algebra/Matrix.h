...
#ifndef ORBITAL_PROPAGATOR_MATRIX_H
#define ORBITAL_PROPAGATOR_MATRIX_H
    ...
    [[nodiscard]] Matrix inverse() const {
        ...
        for (std::size_t i = 0; i < M; ++i) {
            for (std::size_t j = 0; j < M; ++j) {
                if ( (i == j && std::abs(augmented[i][j] - 1.) > 1E-12) || (i != j && std::abs(augmented[i][j]) > 1E-12) )
                    throw std::invalid_argument("Matrix: the inverse does not exist.");
            }
        }
...
#endif //ORBITAL_PROPAGATOR_MATRIX_H
